...
VALUES 
  ((SELECT id FROM "USER" WHERE email = 'lee2000@hexschooltest.io'), 2, NULL, NULL),
  ((SELECT id FROM "USER" WHERE email = 'muscle@hexschooltest.io'), 2, NULL, NULL),
  ((SELECT id FROM "USER" WHERE email = 'starplatinum@hexschooltest.io'), 2, NULL, NULL)
;
...
Select 
   ...
   s.id
FROM "USER" as u 
JOIN "COACH" as c ON u.id = c.user_id
...
FROM "USER" as u 
JOIN "COACH" as c ON u.id = c.user_id
JOIN "SKILL" as s ON s.name IN ('有氧運動', '復健訓練')
...
UPDATE "COACH"
	SET experience_years = 3,
	  updated_at = CURRENT_TIMESTAMP
WHERE 
	user_id = ((select id from "USER" where email = 'muscle@hexschooltest.io' AND role = 'COACH'))
;
UPDATE "COACH"
	SET experience_years = 5,
	  updated_at = CURRENT_TIMESTAMP
WHERE 
	user_id = ((select id from "USER" where email = 'starplatinum@hexschooltest.io' AND role = 'COACH'))
;
...
-- using email to find user_id, sill name to find skill_id
-- USER - COACH - COACH LINK - COURSE
...
FROM "USER" as u 
JOIN "COACH" as c ON u.id = c.user_id
JOIN "COACH_LINK_SKILL" as co ON c.id = co.coach_id
join "SKILL" as s ON s.id = co.skill_id
...
VALUES (
    ...
    (SELECT c.id 
     FROM "USER" u
     JOIN "COURSE" c ON u.id = c.user_id
     WHERE u.email = 'lee2000@hexschooltest.io'),
    '2024-11-24 16:00:00',
    '即將授課'
);
...
WHERE 
	id  = (
        SELECT
            a.id AS course_booking_id
        FROM "COURSE_BOOKING" AS a
        LEFT JOIN "USER" AS b 
            ON a.user_id = b.id
        ...
        LEFT JOIN "USER" AS u
            ON c.user_id = u.id
        WHERE 
            b.email = 'wXlTq@hexschooltest.io' AND
            u.email = 'lee2000@hexschooltest.io' AND
            c.name = '重訓基礎課'
	)
;
...
left join "USER" as b 
    on a.user_id = b.id 
WHERE b.email = 'wXlTq@hexschooltest.io'
;
...
WHERE 
	id  = (
        ...
        LEFT JOIN "USER" AS u
            ON c.user_id = u.id
        WHERE 
            b.email = 'wXlTq@hexschooltest.io' 
            AND u.email = 'lee2000@hexschooltest.io' 
            AND c.name = '重訓基礎課'
...
SELECT 
    a.user_id,
    SUM(a.purchased_credits) AS total_classes
...
SELECT 
    a.user_id,
    COUNT(a.id) AS total_used_classes
FROM "COURSE_BOOKING" AS a
JOIN "USER" AS b 
    ON a.user_id = b.id
WHERE a.cancelled_at IS NOT NULL 
  AND b.email = 'wXlTq@hexschooltest.io'
GROUP BY a.user_id
;
...
SELECT 
    t1.user_id,
    (t1.total_credit - t2.used_credit) AS remaining_credit
FROM (
    SELECT 
        a.user_id,
...
SELECT 
    ...
    s.name AS "專長名稱"
FROM "SKILL" AS s
INNER JOIN "COACH_LINK_SKILL" AS cls 
    ON s.id = cls.skill_id
INNER JOIN "COACH" AS c 
    ON cls.coach_id = c.id
...
SELECT 
    ...
    COUNT(a.id) AS "銷售數量"
FROM "CREDIT_PURCHASE" AS a
LEFT JOIN "CREDIT_PACKAGE" AS b
    ON a.credit_package_id = b.id
WHERE a.purchase_at::DATE >= '2024-11-01'
  AND a.purchase_at::DATE < '2024-12-01'
GROUP BY b."name"
;
...
WHERE purchase_at::DATE >= '2024-11-01'
  AND purchase_at::DATE < '2024-12-01'
;
...
FROM "COURSE_BOOKING"
WHERE created_at::DATE BETWEEN '2024-11-01' AND '2024-11-30'
  AND status IN ('即將上課', '上課中')
;
